...
const components = {
  ...
  types: {
    image: ({value}: any) => {
      if (!value?.asset) return null; // Zabezpieczenie zdjęć wewnątrz tekstu
      return (
...
export default async function PostPage({ params }: { params: { slug: string } }) {
  // Dodajemy catch, aby obsłużyć błędy sieciowe
  const post = await client.fetch(POST_QUERY, { slug: params.slug }, { cache: 'no-store' }).catch(() => null);

  if (!post) {
    return <div className="p-10 text-center font-bold">Article not found or is being updated...</div>;
  }
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          <article className="lg:w-2/3 bg-white p-6 md:p-10 shadow-sm border border-gray-200">
            <header className="mb-8 border-b border-gray-100 pb-8">
              <div className="mb-4">
                <span className="text-[10px] font-black uppercase text-white bg-orange-600 px-3 py-1 tracking-widest">
                  {post?.category || "Crypto News"}
                </span>
              </div>
              <h1 className="text-3xl md:text-5xl font-black leading-tight text-black mb-6 tracking-tighter">
            ...
            {/* Zabezpieczenie zdjęcia głównego */}
